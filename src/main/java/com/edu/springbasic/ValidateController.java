...
public class ValidateController {
	...
	두번째 파라미터
		:BindingResult객체는 validator를 통해 폼값을 검증한 후
		해당 결과를 전송받는 객체로, 폼값에 문제가 있다면 해당 객체를 통해 확인할 수 있다.
	세번째 파라미터
		: 데이터를 저장할 Model객체로 View로 전달하는 역할을 한다.
		톰캣에서 제공하는 request영역과 동일한 기능의 객체이다.
	*/
	...
	public String registProc(
		...
		유효성 체크를 할 커맨드 객체와 바인딩 결과(검증결과)를 저장할 객체를 인수로 전달한다.
		*/
